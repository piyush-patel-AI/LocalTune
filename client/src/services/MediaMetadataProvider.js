...
export function getArtworkUrl(track, requestedSize = 512) {
  ...
  // Return standard cover art endpoint with optional size parameter
  return `/api/tracks/${track.id}/art?size=${requestedSize}`;
}
...
export function updateMediaSessionPositionState(positionData = {}) {
  ...
  try {
    const { duration = 0, playbackRate = 1, position = 0 } = positionData;
